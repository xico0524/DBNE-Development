...
public class M6_PLogBN extends Ensemble_init{
	...
	public double[]	dev_log_i;
	public double[] dev_log_j; 
	public double[] dev_log_k; 
	...
	public void DriftLogBias() 
	{
		alpha = new double[uNum+1];
		beta = new double[sNum+1];
		gama = new double[tNum+1];	
		
		dev_log_i = new double[uNum+1];
		dev_log_j = new double[sNum+1];
		dev_log_k = new double[tNum+1];
		...
		for(int i=1; i<=uNum; i++) 
		{
			...
			dev_log_i[i] = Math.log(i) + w1;
		}
		
		for(int j=1; j<=sNum; j++) 
		{
			dev_log_j[j] = Math.log(j) + w2;
		}

		for(int k=1; k<=tNum; k++) 
		{
			dev_log_k[k] = Math.log(k) + w3;
		}
	...
	public void train(int rank)
	{
		...
		for(int tr=1; tr<=trainingRound; tr++)
		{
				...
				for(int i=1; i<=uNum; i++) 
				{
					...
					if(USlice.containsKey(Integer.valueOf(i))) 
					{
						...
						for(RTuple utemp: uSlice) 
						{
							...
							uDownTemp += alpha[i]*dev_log_i[i] + beta[utemp.rowID]*dev_log_j[utemp.rowID] + gama[utemp.colID]*dev_log_k[utemp.colID];
				...
				for(int j=1; j<=sNum; j++) 
				{
					...
					if(SSlice.containsKey(Integer.valueOf(j))) 
					{
						...
						for(RTuple stemp: sSlice) 
						{
							...
							sDownTemp += alpha[stemp.rowID]*dev_log_i[stemp.rowID] + beta[j]*dev_log_j[j] + gama[stemp.colID]*dev_log_k[stemp.colID];
				...
				for(int k=1; k<=tNum; k++) 
				{
					...
					if(TSlice.containsKey(Integer.valueOf(k))) 
					{
						...
						for(RTuple ttemp: tSlice) 
						{
							...
							tDownTemp += alpha[ttemp.rowID]*dev_log_i[ttemp.rowID] + beta[ttemp.colID]*dev_log_j[ttemp.colID] + gama[k]*dev_log_k[k];
			...
			for(int i=1; i<=uNum;i++) 
			{
				...
				if(USlice.containsKey(Integer.valueOf(i)))
				{
					...
					for(RTuple utemp: uSlice) 
					{
						...
						aDownTemp += alpha[i]*dev_log_i[i] + beta[utemp.rowID]*dev_log_j[utemp.rowID] + gama[utemp.colID]*dev_log_k[utemp.colID];
						
						aDown += aDownTemp * dev_log_i[i];
			...
			for(int j=1; j<=sNum;j++) 
			{
				...
				if(SSlice.containsKey(Integer.valueOf(j)))
				{
					...
					for(RTuple stemp: sSlice) 
					{
						...
						bDownTemp += alpha[stemp.rowID]*dev_log_i[stemp.rowID] + beta[j]*dev_log_j[j] + gama[stemp.colID]*dev_log_k[stemp.colID];
						
						bDown += bDownTemp * dev_log_j[j];
			...
			for(int k=1; k<=tNum;k++) 
			{
				...
				if(TSlice.containsKey(Integer.valueOf(k)))
				{
					...
					for(RTuple ttemp: tSlice) 
					{
						...
						cDownTemp += alpha[ttemp.rowID]*dev_log_i[ttemp.rowID] + beta[ttemp.colID]*dev_log_j[ttemp.colID] + gama[k]*dev_log_k[k];
						cDown += cDownTemp * dev_log_k[k];
			...
			for(Quadrup valid : validData) 
			{
				...
				ytemp += alpha[valid.uID]*dev_log_i[valid.uID] + beta[valid.sID]*dev_log_j[valid.sID] + gama[valid.tID]*dev_log_k[valid.tID];
		...
		System.out.println("\nM6 Validation Convergence Round: "+convergenceRound);
		...
		for (Quadrup test: testData)
		{
			...
			ytemp_RMSE += alpha_update_RMSE[test.uID]*dev_log_i[test.uID] + beta_update_RMSE[test.sID]*dev_log_j[test.sID] + gama_update_RMSE[test.tID]*dev_log_k[test.tID];
			ytemp_MAE += alpha_update_MAE[test.uID]*dev_log_i[test.uID] + beta_update_MAE[test.sID]*dev_log_j[test.sID] + gama_update_MAE[test.tID]*dev_log_k[test.tID]; 
			ytemp_NSE += alpha_update_NSE[test.uID]*dev_log_i[test.uID] + beta_update_NSE[test.sID]*dev_log_j[test.sID] + gama_update_NSE[test.tID]*dev_log_k[test.tID]; 
	...
	public double[] boosting() 
	{
		...
		for(Quadrup temp : trainData) 
		{
			...
			ytemp_RMSE += alpha_update_RMSE[temp.uID]*dev_log_i[temp.uID] + beta_update_RMSE[temp.sID]*dev_log_j[temp.sID] + gama_update_RMSE[temp.tID]*dev_log_k[temp.tID]; 
			ytemp_MAE += alpha_update_MAE[temp.uID]*dev_log_i[temp.uID] + beta_update_MAE[temp.sID]*dev_log_j[temp.sID] + gama_update_MAE[temp.tID]*dev_log_k[temp.tID]; 
			ytemp_NSE += alpha_update_NSE[temp.uID]*dev_log_i[temp.uID] + beta_update_NSE[temp.sID]*dev_log_j[temp.sID] + gama_update_NSE[temp.tID]*dev_log_k[temp.tID]; 
		...
		double[] result_M6_omega = new double[3];
		result_M6_omega[0] = omega_RMSE;
		result_M6_omega[1] = omega_MAE;
		result_M6_omega[2] = omega_NSE;
		
		System.out.println("Weight: "+ Arrays.toString(result_M6_omega));
