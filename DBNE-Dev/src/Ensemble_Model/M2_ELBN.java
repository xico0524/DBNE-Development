...
public class M2_ELBN extends Ensemble_init{
	...
	public void extendedBias() 
	{
		A = new double[uNum+1][bias_dimen+1];
		B = new double[sNum+1][bias_dimen+1];
		C = new double[tNum+1][bias_dimen+1];
		
		Random randoma = new Random(System.currentTimeMillis());
		for(int i=1; i<=uNum;i++) 
		{
			for(int l=1; l<=bias_dimen;l++) 
			{
				A[i][l] = biasMin + randoma.nextDouble() * (biasMax - biasMin);
			}
		}
		...
		{	
			for(int l=1; l<=bias_dimen;l++)
			{
				B[j][l] = biasMin + randomb.nextDouble() * (biasMax - biasMin);
			}
		}
		...
		for(int k=1; k<=tNum; k++) 
		{
			for(int l=1; l<=bias_dimen;l++) 
			{
				C[k][l] = biasMin + randomc.nextDouble() * (biasMax - biasMin);
			}
		}
	}
	...
	public void train(int rank)
	{
		...
		A_update_RMSE = new double[uNum+1][bias_dimen+1];
		B_update_RMSE = new double[sNum+1][bias_dimen+1];
		C_update_RMSE = new double[tNum+1][bias_dimen+1];
		
		U_update_MAE = new double[uNum+1][rank+1];
		S_update_MAE = new double[sNum+1][rank+1];
		T_update_MAE = new double[tNum+1][rank+1];
		A_update_MAE = new double[uNum+1][bias_dimen+1];
		B_update_MAE = new double[sNum+1][bias_dimen+1];
		C_update_MAE = new double[tNum+1][bias_dimen+1];
		
		U_update_NSE = new double[uNum+1][rank+1];
		S_update_NSE = new double[sNum+1][rank+1];
		T_update_NSE = new double[tNum+1][rank+1];
		A_update_NSE = new double[uNum+1][bias_dimen+1];
		B_update_NSE = new double[sNum+1][bias_dimen+1];
		C_update_NSE = new double[tNum+1][bias_dimen+1];
		...
		for(int tr=1; tr<=trainingRound; tr++)
		{
				...
				for(int i=1; i<=uNum; i++) 
				{
					...
					if(USlice.containsKey(Integer.valueOf(i))) 
					{
						...
						for(RTuple utemp: uSlice) 
						{
							...
							for(int r_bias=1; r_bias<=bias_dimen; r_bias++) 
							{
								uDownTemp += A[i][r_bias] + B[utemp.rowID][r_bias] + C[utemp.colID][r_bias];
							}
				...
				for(int j=1; j<=sNum; j++) 
				{
					...
					if(SSlice.containsKey(Integer.valueOf(j))) 
					{
						...
						for(RTuple stemp: sSlice) 
						{
							...
							for(int r_bias=1; r_bias<=bias_dimen; r_bias++) 
							{
								sDownTemp += A[stemp.rowID][r_bias] + B[j][r_bias] + C[stemp.colID][r_bias];
							}
				...
				for(int k=1; k<=tNum; k++) 
				{
					...
					if(TSlice.containsKey(Integer.valueOf(k))) 
					{
						...
						for(RTuple ttemp: tSlice) 
						{
							...
							for(int r_bias=1; r_bias<=bias_dimen; r_bias++) 
							{
								tDownTemp += A[ttemp.rowID][bias_dimen] + B[ttemp.colID][bias_dimen] + C[k][bias_dimen];
							}
			...
			for(int i=1; i<=uNum;i++) 
			{
				...
				if(USlice.containsKey(Integer.valueOf(i)))
				{
					...
					for(RTuple utemp: uSlice) 
					{
						...
						for(int r_bias=1; r_bias<=bias_dimen; r_bias++) 
						{
							aDown += A[i][r_bias] + B[utemp.rowID][r_bias] + C[utemp.colID][r_bias];
							aDown += lambda_b * A[i][r_bias];
						}
					}
				}
				...
				for(int r_bias_u=1; r_bias_u<=bias_dimen;r_bias_u++) 
				{
					aUp = A[i][r_bias_u] * aUp;
					A[i][r_bias_u] = aUp / aDown;
				}
			...
			for(int j=1; j<=sNum;j++) 
			{
				...
				if(SSlice.containsKey(Integer.valueOf(j)))
				{
					...
					for(RTuple stemp: sSlice) 
					{
						...
						for(int r_bias=1; r_bias<=bias_dimen; r_bias++)
						{
							bDown += A[stemp.rowID][r_bias] + B[j][r_bias] + C[stemp.colID][r_bias];
							bDown += lambda_b * B[j][r_bias];
						}
					}
				}
				...
				for(int r_bias_s=1; r_bias_s<=bias_dimen;r_bias_s++) 
				{
					bUp = B[j][r_bias_s] * bUp;
					B[j][r_bias_s] = bUp / bDown;
				}
			...
			for(int k=1; k<=tNum;k++) 
			{
				...
				if(TSlice.containsKey(Integer.valueOf(k)))
				{
					...
					for(RTuple ttemp: tSlice) 
					{
						...
						for(int r_bias=1; r_bias<=bias_dimen; r_bias++) 
						{
							cDown += A[ttemp.rowID][r_bias] + B[ttemp.colID][r_bias] + C[k][r_bias];
							cDown += lambda_b * C[k][r_bias];
						}
					}
				}
				...
				for(int r_bias_t=1; r_bias_t<=bias_dimen;r_bias_t++)
				{
					cUp = C[k][r_bias_t] * cUp;
					C[k][r_bias_t] = cUp / cDown;
				}
			...
			for(Quadrup valid : validData) 
			{
				...
				for(int yl=1; yl<=bias_dimen; yl++) 
				{
					ytemp += A[valid.uID][yl] + B[valid.sID][yl] + C[valid.tID][yl];
				}				
		...
		System.out.println("\nM2 Validation Convergence Round: "+convergenceRound);
		...
		for (Quadrup test: testData)
		{
			...
			for(int yl=1; yl<=bias_dimen; yl++) 
			{
				ytemp_RMSE += A_update_RMSE[test.uID][yl] + B_update_RMSE[test.sID][yl] + C_update_RMSE[test.tID][yl];
				ytemp_MAE += A_update_MAE[test.uID][yl] + B_update_MAE[test.sID][yl] + C_update_MAE[test.tID][yl]; 
				ytemp_NSE += A_update_NSE[test.uID][yl] + B_update_NSE[test.sID][yl] + C_update_NSE[test.tID][yl];
			}
	...
	public double[] boosting() 
	{
		...
		for(Quadrup temp : trainData) 
		{
			...
			for(int yl=1; yl<=bias_dimen; yl++) 
			{
				ytemp_RMSE += A_update_RMSE[temp.uID][yl] + B_update_RMSE[temp.sID][yl] + C_update_RMSE[temp.tID][yl];
				ytemp_MAE += A_update_MAE[temp.uID][yl] + B_update_MAE[temp.sID][yl] + C_update_MAE[temp.tID][yl];
				ytemp_NSE += A_update_NSE[temp.uID][yl] + B_update_NSE[temp.sID][yl] + C_update_NSE[temp.tID][yl];
			}			
		...
		double[] result_M2_omega = new double[3];
		result_M2_omega[0] = omega_RMSE;
		result_M2_omega[1] = omega_MAE;
		result_M2_omega[2] = omega_NSE;
		
		
		System.out.println("Weight: "+ Arrays.toString(result_M2_omega));
