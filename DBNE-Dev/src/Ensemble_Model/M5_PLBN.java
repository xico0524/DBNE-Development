...
public class M5_PLBN extends Ensemble_init{
	
	public double lambda = 1.0E-3;
	public double lambda_b = 1.0E-5;
	
	public double w1 = 0.1;
	public double w2 = w1;
	public double w3 = w1;
	...
	public double[]	dev_linear_i;
	public double[] dev_linear_j; 
	public double[] dev_linear_k; 
	...
	public void DriftLinearBias() 
	{
		alpha = new double[uNum+1];
		beta = new double[sNum+1];
		gama = new double[tNum+1];	
		
		dev_linear_i = new double[uNum+1];
		dev_linear_j = new double[sNum+1];
		dev_linear_k = new double[tNum+1];
		...
		for(int i=1; i<=uNum; i++) 
		{
			...
			{
				dev_linear_i[i] = Math.pow(i - 1, w1);
			}
		}
		...
		for(int j=1; j<=sNum; j++) 
		{
			...
			{
				dev_linear_j[j] = Math.pow(j - 1, w2);
			}
		}
		...
		for(int k=1; k<=tNum; k++) 
		{
			...
			{
				dev_linear_k[k] = Math.pow(k - 1, w3);
			}
		}
	}
	...
	public void train(int rank)
	{
		...
		for(int tr=1; tr<=trainingRound; tr++)
		{
				...
				for(int i=1; i<=uNum; i++) 
				{
					...
					if(USlice.containsKey(Integer.valueOf(i))) 
					{
						...
						for(RTuple utemp: uSlice) 
						{
							...
							uDownTemp += alpha[i]*dev_linear_i[i] + beta[utemp.rowID]*dev_linear_j[utemp.rowID] + gama[utemp.colID]*dev_linear_k[utemp.colID];
				...
				for(int j=1; j<=sNum; j++) 
				{
					...
					if(SSlice.containsKey(Integer.valueOf(j))) 
					{
						...
						for(RTuple stemp: sSlice) 
						{
							...
							sDownTemp += alpha[stemp.rowID]*dev_linear_i[stemp.rowID] + beta[j]*dev_linear_j[j] + gama[stemp.colID]*dev_linear_k[stemp.colID];
				...
				for(int k=1; k<=tNum; k++) 
				{
					...
					if(TSlice.containsKey(Integer.valueOf(k))) 
					{
						...
						for(RTuple ttemp: tSlice) 
						{
							...
							tDownTemp += alpha[ttemp.rowID]*dev_linear_i[ttemp.rowID] + beta[ttemp.colID]*dev_linear_j[ttemp.colID] + gama[k]*dev_linear_k[k];
			...
			for(int i=1; i<=uNum;i++) 
			{
				...
				if(USlice.containsKey(Integer.valueOf(i)))
				{
					...
					for(RTuple utemp: uSlice) 
					{
						aUp += utemp.mvalue * dev_linear_i[i];
						...
						aDownTemp += alpha[i]*dev_linear_i[i] + beta[utemp.rowID]*dev_linear_j[utemp.rowID] + gama[utemp.colID]*dev_linear_k[utemp.colID];
						
						aDown += aDownTemp * dev_linear_i[i];
			...
			for(int j=1; j<=sNum;j++) 
			{
				...
				if(SSlice.containsKey(Integer.valueOf(j)))
				{
					...
					for(RTuple stemp: sSlice) 
					{
						bUp += stemp.mvalue * dev_linear_j[j];
						...
						bDownTemp += alpha[stemp.rowID]*dev_linear_i[stemp.rowID] + beta[j]*dev_linear_j[j] + gama[stemp.colID]*dev_linear_k[stemp.colID];
						
						bDown += bDownTemp * dev_linear_j[j];
			...
			for(int k=1; k<=tNum;k++) 
			{
				...
				if(TSlice.containsKey(Integer.valueOf(k)))
				{
					...
					for(RTuple ttemp: tSlice) 
					{
						cUp += ttemp.mvalue * dev_linear_k[k];
						...
						cDownTemp += alpha[ttemp.rowID]*dev_linear_i[ttemp.rowID] + beta[ttemp.colID]*dev_linear_j[ttemp.colID] + gama[k]*dev_linear_k[k];
						cDown += cDownTemp * dev_linear_k[k];
			...
			for(Quadrup valid : validData) 
			{
				...
				ytemp += alpha[valid.uID]*dev_linear_i[valid.uID] + beta[valid.sID]*dev_linear_j[valid.sID] + gama[valid.tID]*dev_linear_k[valid.tID];
		...
		System.out.println("\nM5 Validation Convergence Round: "+convergenceRound);
		...
		for (Quadrup test: testData)
		{
			...
			ytemp_RMSE += alpha_update_RMSE[test.uID]*dev_linear_i[test.uID] + beta_update_RMSE[test.sID]*dev_linear_j[test.sID] + gama_update_RMSE[test.tID]*dev_linear_k[test.tID];
			ytemp_MAE += alpha_update_MAE[test.uID]*dev_linear_i[test.uID] + beta_update_MAE[test.sID]*dev_linear_j[test.sID] + gama_update_MAE[test.tID]*dev_linear_k[test.tID]; 
			ytemp_NSE += alpha_update_NSE[test.uID]*dev_linear_i[test.uID] + beta_update_NSE[test.sID]*dev_linear_j[test.sID] + gama_update_NSE[test.tID]*dev_linear_k[test.tID]; 
	...
	public double[] boosting() 
	{
		...
		for(Quadrup temp : trainData) 
		{
			...
			ytemp_RMSE += alpha_update_RMSE[temp.uID]*dev_linear_i[temp.uID] + beta_update_RMSE[temp.sID]*dev_linear_j[temp.sID] + gama_update_RMSE[temp.tID]*dev_linear_k[temp.tID]; 
			ytemp_MAE += alpha_update_MAE[temp.uID]*dev_linear_i[temp.uID] + beta_update_MAE[temp.sID]*dev_linear_j[temp.sID] + gama_update_MAE[temp.tID]*dev_linear_k[temp.tID]; 
			ytemp_NSE += alpha_update_NSE[temp.uID]*dev_linear_i[temp.uID] + beta_update_NSE[temp.sID]*dev_linear_j[temp.sID] + gama_update_NSE[temp.tID]*dev_linear_k[temp.tID]; 
		...
		double[] result_M5_omega = new double[3];
		result_M5_omega[0] = omega_RMSE;
		result_M5_omega[1] = omega_MAE;
		result_M5_omega[2] = omega_NSE;
		
		System.out.println("Weight: "+ Arrays.toString(result_M5_omega));
