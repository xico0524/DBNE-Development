...
public class M3_WPBN extends Ensemble_init{
	
	public double lambda = 1.0E-3;
	public double lambda_b = 1.0E-5;
	
	public int theta1 = 10;
	public int theta2 = theta1;
	public int theta3 = theta1;
	...
	public void WeightedPreBias() 
	{
		...
		for(int i=1; i<=uNum; i++)
		{
			...
			{
				ArrayList<RTuple> aSlice = new ArrayList<RTuple>(USlice.get(Integer.valueOf(i)));
				
				for(RTuple atemp : aSlice) 
				{
					aReal += atemp.mvalue;
					aNum++;
				}
				A[i] = (aReal - miu*aNum) / (theta1 + aNum);
			}
		...
		for(int j=1; j<=sNum; j++) 
		{
			...
			{
				ArrayList<RTuple> bSlice = new ArrayList<RTuple>(SSlice.get(Integer.valueOf(j)));
				
				for(RTuple btemp : bSlice)
				{
					bReal += btemp.mvalue;
					bNum++;
				}
				B[j] = (bReal - miu*bNum) / (theta2 + bNum);
			}
		...
		for(int k=1; k<=tNum; k++) 
		{
			...
			{
				ArrayList<RTuple> cSlice = new ArrayList<RTuple>(TSlice.get(Integer.valueOf(k)));
				
				for(RTuple ctemp : cSlice)
				{
					cReal += ctemp.mvalue;
					cNum++;
				}
				C[k] = (cReal - miu*cNum) / (theta3 + cNum);
			}
	...
	public void train(int rank)
	{
		...
		for(int tr=1; tr<=trainingRound; tr++)
		{
				...
				for(int i=1; i<=uNum; i++) 
				{
					...
					if(USlice.containsKey(Integer.valueOf(i))) 
					{
						...
						for(RTuple utemp: uSlice) 
						{
							...
							uDownTemp += alpha[i]*A[i] + beta[utemp.rowID]*B[utemp.rowID] + gama[utemp.colID]*C[utemp.colID];
				...
				for(int j=1; j<=sNum; j++) 
				{
					...
					if(SSlice.containsKey(Integer.valueOf(j))) 
					{
						...
						for(RTuple stemp: sSlice) 
						{
							...
							sDownTemp += alpha[stemp.rowID]*A[stemp.rowID] + beta[j]*B[j] + gama[stemp.colID]*C[stemp.colID];
				...
				for(int k=1; k<=tNum; k++) 
				{
					...
					if(TSlice.containsKey(Integer.valueOf(k))) 
					{
						...
						for(RTuple ttemp: tSlice) 
						{
							...
							tDownTemp += alpha[ttemp.rowID]*A[ttemp.rowID] + beta[ttemp.colID]*B[ttemp.colID] + gama[k]*C[k];
			...
			for(int i=1; i<=uNum;i++) 
			{
				...
				if(USlice.containsKey(Integer.valueOf(i)))
				{
					...
					for(RTuple utemp: uSlice) 
					{
						...
						aDownTemp += alpha[i]*A[i] + beta[utemp.rowID]*B[utemp.rowID] + gama[utemp.colID]*C[utemp.colID];
						aDown += aDownTemp * A[i];
			...
			for(int j=1; j<=sNum;j++) 
			{
				...
				if(SSlice.containsKey(Integer.valueOf(j)))
				{
					...
					for(RTuple stemp: sSlice) 
					{
						...
						bDownTemp += alpha[stemp.rowID]*A[stemp.rowID] + beta[j]*B[j] + gama[stemp.colID]*C[stemp.colID];
						bDown += bDownTemp * B[j];
			...
			for(int k=1; k<=tNum;k++) 
			{
				...
				if(TSlice.containsKey(Integer.valueOf(k)))
				{
					...
					for(RTuple ttemp: tSlice) 
					{
						...
						cDownTemp += alpha[ttemp.rowID]*A[ttemp.rowID] + beta[ttemp.colID]*B[ttemp.colID] + gama[k]*C[k];
						cDown += cDownTemp * C[k];
			...
			for(Quadrup valid : validData) 
			{
				...
				ytemp += alpha[valid.uID]*A[valid.uID] + beta[valid.sID]*B[valid.sID] + gama[valid.tID]*C[valid.tID];
		...
		System.out.println("\nM3 Validation Convergence Round: "+convergenceRound);
		...
		for (Quadrup test: testData)
		{
			...
			ytemp_RMSE += alpha_update_RMSE[test.uID]*A[test.uID] + beta_update_RMSE[test.sID]*B[test.sID] + gama_update_RMSE[test.tID]*C[test.tID];
			ytemp_MAE += alpha_update_MAE[test.uID]*A[test.uID] + beta_update_MAE[test.sID]*B[test.sID] + gama_update_MAE[test.tID]*C[test.tID]; 
			ytemp_NSE += alpha_update_NSE[test.uID]*A[test.uID] + beta_update_NSE[test.sID]*B[test.sID] + gama_update_NSE[test.tID]*C[test.tID]; 
	...
	public double[] boosting() 
	{
		...
		for(Quadrup temp : trainData) 
		{
			...
			ytemp_RMSE += alpha_update_RMSE[temp.uID]*A[temp.uID] + beta_update_RMSE[temp.sID]*B[temp.sID] + gama_update_RMSE[temp.tID]*C[temp.tID];
			ytemp_MAE += alpha_update_MAE[temp.uID]*A[temp.uID] + beta_update_MAE[temp.sID]*B[temp.sID] + gama_update_MAE[temp.tID]*C[temp.tID]; 
			ytemp_NSE += alpha_update_NSE[temp.uID]*A[temp.uID] + beta_update_NSE[temp.sID]*B[temp.sID] + gama_update_NSE[temp.tID]*C[temp.tID]; 
		...
		double[] result_M3_omega = new double[3];
		result_M3_omega[0] = omega_RMSE;
		result_M3_omega[1] = omega_MAE;
		result_M3_omega[2] = omega_NSE;
		
		System.out.println("Weight: "+ Arrays.toString(result_M3_omega));
